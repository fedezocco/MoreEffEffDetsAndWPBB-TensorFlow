...
class ModelInspector(object):
  ...
  def benchmark_model(self,
    ...
    with tf.Graph().as_default(), tf.Session(config=sess_config) as sess:
      ...
      ## Block modified by Federico Zocco (takes the median rather than the average time since it is more robust against outliers)
      import statistics
      import numpy 
      vector_of_times = -123*numpy.ones(bm_runs) 
      for i in range(bm_runs):
        start = time.perf_counter()
        sess.run(output_name, feed_dict={input_name: img})
        end = time.perf_counter()
        vector_of_times[i] = end - start
      inference_time = statistics.median(vector_of_times)
      print('Per batch inference time: ', inference_time)
      print('FPS: ', self.batch_size / inference_time)
      print('vector of times: ', vector_of_times)
